...
public interface PlayerRepository extends JpaRepository<Player, Long> {
	...
	@Query(value = "SELECT p.user_id,pd.points FROM myipl.player_prediction pd INNER JOIN myipl.player p on p.user_id = pd.user_id INNER JOIN myipl.iplgroup g ON g.id = p.group_id WHERE p.group_id = ( select p1.group_id  FROM myipl.player p1 where p1.user_id =:userId ) order by pd.points desc, p.user_id", nativeQuery = true)
